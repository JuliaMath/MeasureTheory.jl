...
\chad{Above sentence is very long, can we break it up?}
...
The practical importance of $\sigma$-algebras here lies therein that they also encode available information. Observe that each event $A$ corresponds to a question, with a probabilistic answer. 
 ...
 This is a very relevant question for probabilistic programming tasks and causal inference, and with \emph{kernels} (Section~\ref{Kernels}) \verb|MeasureTheory| does provide tools to reason about it which are computationally appropriate, because they are local.
 
\chad{above paragraph needs editing} 
...
Note the difference from a typical distribution-oriented implementation; we have changed the base measure in order to simplify the log-density. This leads to better compositional efficiency. \chad{"compositional efficiency" is kind of vague, reword}
...
We could easily have chosen a different base measure and adjusted \verb|logdensity_def| accordingly.
From the perspective of an end user, this could be considered arbitrary.
Therefore, \verb|basemeasure| and \verb|logdensity_def| should be considered implementation details, and not part of the user-facing interface.
...
Of particular interest for the base measure is the case
\[
...
Note that a likelihood $\ell$ takes a point in $\Theta$ to an evaluated density in a different space (the observation space $X$). In particular, a likelihood is neither a measure nor a density.

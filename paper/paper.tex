...
Instead of \verb|logdensity_def|, users should typically call \verb|logdensity_rel(μ,ν,x)| (which computes $\frac{\dd μ}{\dd ν}(x)$), or \verb|logdensityof(μ,x)|, which gives the log-density relative to the root measure. \verb|logdensityof| was originally defined in \cite{DensityInterface.jl}.

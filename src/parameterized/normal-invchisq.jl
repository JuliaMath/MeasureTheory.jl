...
function MeasureBase.insupport(d::NormalInvChiSq, x::NamedTuple{(:μ, :σ), Tuple{M,S}}) where {M<:Real,S<:Real}
    x.σ > 0
end

# See Gelman et al BDA 2nd edition, section 3.3
function MeasureBase.logdensity_def(d::NormalInvChiSq{(:μ, :σ², :κ, :ν)}, x::NamedTuple{(:μ, :σ)})
    (μ₀, σ²₀, κ₀, ν₀) = params(d)
...
# See Gelman et al BDA 2nd edition, section 3.3
function Base.rand(rng::AbstractRNG, ::Type{T}, d::NormalInvChiSq{(:μ, :σ², :κ, :ν)}) where {T}
    (μ₀, σ²₀, κ₀, ν₀) = params(d)
